...
import { Workflow as WorkflowIcon, Play, Loader2, Check, X, Clock, Cpu, ArrowRight, Zap, Plus, Trash2, Save } from 'lucide-react'
...
const STEP_ICONS: Record<string, string> = { generate_document: '📝', index_in_rag: '📚', save_generation: '💾', translate: '🌍', summarize: '📋', improve: '✨', extract_keywords: '🔑', custom: '⚙️' }
        ...
        <div className="space-y-3">
          ...
          {workflows.map(w => (
            <Card key={w.id} className={`p-4 cursor-pointer transition-all ${selected?.id === w.id ? 'border-2 border-[#27698a]' : 'hover:border-[#27698a]/40'}`} onClick={() => { setSelected(w); setResults(null); setInput('') }}>
              ...
              <div className="flex items-center gap-1 flex-wrap">{(w.steps || []).slice(0, 5).map((s, i) => <div key={s.id} className="flex items-center gap-1"><Badge variant="outline" className={`text-[8px] ${STEP_COLORS[s.type] || 'bg-slate-100'}`}>{STEP_ICONS[s.type] || '⚙️'} {s.label}</Badge>{i < (w.steps || []).length - 1 && <ArrowRight className="w-2.5 h-2.5 text-slate-300" />}</div>)}</div>
            </Card>
          ))}
          ...
          {customWorkflows.map((w: any) => (
            <Card key={w.id} className={`p-3 cursor-pointer transition-all ${selected?.id === w.id ? 'border-2 border-[#27698a]' : 'hover:border-[#27698a]/40'}`} onClick={() => { setSelected({ ...w, isCustom: true, stepsCount: w.steps.length }); setResults(null); setInput('') }}>
              <div className="flex items-start justify-between mb-1"><span className="text-xs font-semibold text-slate-900">{w.name}</span>{w.trigger && w.trigger !== 'manual' && <Badge variant="outline" className="text-[8px] bg-amber-50 text-amber-700">⚡ {w.trigger}</Badge>}</div>
            ...
            <>
              ...
              {results.length > 0 && (
                <Card className="p-4">
                  ...
                  <div className="space-y-2 mb-4">{results.map((r, idx) => { const isExpanded = expandedStep === r.stepId; return <div key={r.stepId} className={`p-3 rounded border ${r.success ? 'border-slate-200' : 'border-red-200 bg-red-50'}`}><div className="flex items-center gap-2 cursor-pointer" onClick={() => setExpandedStep(isExpanded ? null : r.stepId)}><div className={`w-6 h-6 rounded-full flex items-center justify-center text-white text-[10px] font-bold ${r.success ? 'bg-emerald-500' : 'bg-red-500'}`}>{r.success ? <Check className="w-3 h-3" /> : <X className="w-3 h-3" />}</div><div className="flex-1 min-w-0"><div className="text-xs font-medium text-slate-900">{idx + 1}. {STEP_ICONS[r.stepType]} {r.stepLabel}</div><div className="text-[10px] text-slate-500">{r.durationMs}ms{r.tokensUsed ? ` · ${r.tokensUsed.totalTokens} tok` : ''}</div></div></div>{isExpanded && <div className="mt-3 space-y-2">{r.error ? <div className="text-xs text-red-600 bg-red-50 p-2 rounded">⚠ {r.error}</div> : <><div><div className="text-[9px] text-slate-400 uppercase mb-0.5">Output</div><pre className="text-[10px] text-slate-700 bg-slate-50 p-2 rounded max-h-48 overflow-y-auto whitespace-pre-wrap">{r.output?.slice(0, 1000)}</pre></div></>}</div>}</div> })}</div>
